...
let bubbles = [];
let seaweedPatches = [];
let jellyfishCount = 2; // Reduced from 3
let jellyfishes = [];
...
function createCloud() {
  return {
    ...
    y: random(-height/3, -100),
    z: random(-200, -50),
    size: random(50, 150),
    speed: random(0.2, 0.8),
    segments: floor(random(3, 7))
  };
}
...
function draw() {
  // Blue sky background - brighter than the previous underwater background
  background(210, 60, 90, 0.2);
  ...
  drawSceneBackground(waveform, avgFrequency);
...
function createSceneElements() {
  ...
  // Create seaweed patches (fewer than before)
  for (let i = 0; i < 4; i++) { // Reduced from 8
    ...
    let seaweedCount = floor(random(2, 5)); // Reduced from 3-8
...
function drawSceneBackground(waveform, avgFrequency) {
  ...
  for (let cloud of clouds) {
    ...
    if (cloud.x > width/2 + cloud.size) {
      cloud.x = -width/2 - cloud.size;
      cloud.y = random(-height/3, -50);
    }
    ...
    for (let i = 0; i < cloud.segments; i++) {
      let offsetX = i * (cloud.size / cloud.segments) - cloud.size/2;
      let segmentSize = random(0.8, 1.2) * cloud.size / 2;
      push();
      translate(offsetX, sin(i + frameCount * 0.01) * 5, 0);
      sphere(segmentSize);
      pop();
    }
